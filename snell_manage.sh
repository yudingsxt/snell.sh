...
updateSnell(){
    ...
    # 获取当前版本
    current_version=$(cat ${snell_version_file} | sed 's/v//g')
    echo -e "${Info} 当前 Snell 版本: v${current_version}"

    # 获取最新版本
    latest_version=""
    if [[ "$ver" == "4" ]]; then
        latest_version=$snell_v4_version
    elif [[ "$ver" == "5" ]]; then
        latest_version=$snell_v5_version
    else
        echo -e "${Error} 暂不支持 v${ver} 版本的自动更新。"
        sleep 3s
        startMenu
        return
    fi

    echo -e "${Info} 检测到最新版本: v${latest_version}"
 ...
 ${Green_font_prefix} 10.${Font_color_suffix} 退出脚本"
    ...
    if [[ "$show_v4_to_v5_option" == true ]]; then
        case "$num" in
            ...
            ;;
            *)
            echo -e "请输入正确数字${Yellow_font_prefix}[0-11]${Font_color_suffix}"
            sleep 2s
            startMenu
            ;;
        esac
